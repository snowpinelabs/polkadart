...
// (raw 64-byte signature) next to each input. Used by .github/workflows/release_sr25519.yml.
//
//   SR25519_SIGNING_KEY=<hex key> dart run tool/sign_prebuilt.dart <file>...
//
// SR25519_SIGNING_KEY may be either a 32-byte (64-hex) Ed25519 seed (as produced by
// tool/gen_key.dart) or a 64-byte (128-hex) libsodium-style secret key whose first 32 bytes are the
// seed — both resolve to the same keypair.
//
// The matching public key is kPrebuiltPublicKeyHex in lib/src/backend/prebuilt.dart; the installer
...
Future<void> main(List<String> args) async {
  final hexKey = Platform.environment['SR25519_SIGNING_KEY'];
  if (hexKey == null || hexKey.isEmpty) {
    stderr.writeln(
      'sign_prebuilt: SR25519_SIGNING_KEY (hex Ed25519 seed or secret key) is not set.',
    );
    exitCode = 1;
    return;
  }
  ...
  if (seed == null) {
    stderr.writeln(
      'sign_prebuilt: SR25519_SIGNING_KEY must be a 32-byte (64-hex) seed or a 64-byte (128-hex) '
